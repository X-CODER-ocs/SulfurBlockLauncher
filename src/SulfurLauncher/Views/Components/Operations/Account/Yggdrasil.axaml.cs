...
public partial class YggdrasilAccountViewModel : ObservableObject, IDialogContext, INotifyDataErrorInfo
{
        ...
        string? hostId = null)
    {
        ...
        {
            var exists = server.AuthType == AccountType.Yggdrasil
            ...
            if (!exists) BuiltInServers.Add(server);
        }

        if (SelectedBuiltInServer == null) SelectedBuiltInServer = BuiltInServers[0];
    ...
    public Minecraft.Classes.AuthServer? SelectedBuiltInServer
    {
        ...
        set
        {
            ...
            if (value != null && !string.IsNullOrEmpty(value.ServerUrl)) ServerUrl = value.ServerUrl;
        }
    }
    ...
    private async void AddServer()
    {
        ...
        var result = await OverlayDialog.ShowCustomAsync<AuthServer, AuthServerViewModel, Minecraft.Classes.AuthServer>(
            new AuthServerViewModel(_authServers.ToArray()), _hostId, options);
            ...
            if (yggdrasilAccounts.Any())
            {
                ...
                foreach (var account in yggdrasilAccounts)
                {
                    ...
                    var minecraftAccount = new MinecraftAccount(AccountType.Yggdrasil)
                    {
                        ...
                        Skin = base64,
                        ServerNote =
                            BuiltInServers.FirstOrDefault(x => x.ServerUrl is not null && ServerUrl is not null &&
                                                               UrlHelper.AreUrlsEqual(x.ServerUrl, ServerUrl))
