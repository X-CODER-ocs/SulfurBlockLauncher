...
public partial class StorageViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(SulfurLauncherSizeString))]
    [NotifyPropertyChangedFor(nameof(TotalSizeString))]
    public partial double SulfurLauncherBytesRaw { get; set; }
    ...
    public string SulfurLauncherSizeString => SulfurLauncherBytesRaw.ToHumanReadableSize(1);
    ...
    public ObservableCollection<GameFolderStorageItem> SulfurLauncherFolders { get; } = [];
    ...
    public async Task RefreshStorageDataAsync()
    {
        ...
        SulfurLauncherBytesRaw = 0;
        GameBytesRaw = 0;
        TotalBytesRaw = 0;
        var folders = Data.ConfigEntry.MinecraftFolders.ToList();
        Logger.Info(
            $"[Storage] Refreshing storage usage for {folders.Count} Minecraft folder(s), SulfurLauncher data at {portalPath}.");

        SulfurLauncherFolders.Clear();
        AddFolder(SulfurLauncherFolders, SettingsLanguageManager.Instance.storage_portalData.CurrentValue(), portalPath,
            portalPath);
        ...
        await Task.Run(() =>
        {
            try
            {
                ...
                var externalBedrockBytes = folders.Sum(folder =>
                {
                    ...
                    var name = folder.DetectedLayout.Kind == MinecraftFolderKind.SulfurLauncherMc
                        ? "bedrock_instances"
                        : "bedrock_versions";
                ...
                Dispatcher.UIThread.Post(() =>
                {
                    ...
                    SulfurLauncherBytesRaw = portalRootBytes;
                    SulfurLauncherFolders[0].SizeBytes = portalRootBytes;
                    ...
                    TotalBytesRaw = SulfurLauncherBytesRaw + gameBytes;
                    ...
                    Logger.Info(
                        $"[Storage] Storage usage refreshed in {stopwatch.Elapsed}: SulfurLauncher={portalBytes} bytes, game={gameBytes} bytes.");
                });
            }
            catch (Exception ex)
            {
