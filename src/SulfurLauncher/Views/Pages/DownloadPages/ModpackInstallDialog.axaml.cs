...
public partial class ModpackInstallDialog : UserControl
{
    ...
    {
        (DataContext as ModpackInstallDialogViewModel)?.Install();
    }

    private void SaveAs_Click(object? sender, RoutedEventArgs e)
    {
        (DataContext as ModpackInstallDialogViewModel)?.SaveAs();
    }

    private void Cancel_Click(object? sender, RoutedEventArgs e)
    {
        (DataContext as ModpackInstallDialogViewModel)?.Cancel();
    }
}

public enum ModpackDownloadDestination
{
    Install,
    SaveAs
}

public sealed record ModpackInstallDialogResult(
    ModpackDownloadDestination Destination,
...
public partial class ModpackInstallDialogViewModel : ObservableObject, IDialogContext
{
    public ModpackInstallDialogViewModel(string suggestedInstanceId, bool showSaveAs = true)
    {
        ...
        SelectedMinecraftFolder =
            MinecraftFolders.FirstOrDefault(item => item.Folder == Data.ConfigEntry.DefaultMinecraftFolder)
    ...
    public bool InstanceIdExists => SelectedMinecraftFolder is not null && !HasInvalidInstanceId &&
                                    Directory.Exists(Path.Combine(SelectedMinecraftFolder.Folder.FolderPath,
                                        SelectedMinecraftFolder.Folder.DetectedLayout.Kind ==
    ...
    partial void OnSelectedMinecraftFolderChanged(ModpackInstallFolderItem? value)
    {
        UpdateInstanceIdState();
    }
    ...
    public void Install()
    {
        RequestClose?.Invoke(this, new ModpackInstallDialogResult(ModpackDownloadDestination.Install,
            SelectedMinecraftFolder?.Folder, InstanceId.Trim()));
    }

    public void SaveAs()
    {
        RequestClose?.Invoke(this, new ModpackInstallDialogResult(ModpackDownloadDestination.SaveAs,
            null, null));
    }
