...
internal static class ModpackInstallation
{
        ...
        string? iconUrl, ModpackInstallDialogResult selection)
    {
        ...
        var isSulfurLauncherMc = MinecraftFolderLayout.TryFindSulfurLauncherMcRoot(folder, out var portalMcRoot);
        var installFolder = isSulfurLauncherMc ? Path.Combine(portalMcRoot, "meta") : folder;
        var instancesRoot = isSulfurLauncherMc ? Path.Combine(portalMcRoot, "instances") : null;
        ...
        var temporaryFolder = Path.Combine(Path.GetTempPath(), "SulfurLauncher", "modpacks", Guid.NewGuid().ToString("N"));
        ...
        catch (OperationCanceledException exception)
        {
            ...
            await DeleteSulfurLauncherMcTemporaryLoaderAsync(instancesRoot, installFolder, instanceId);
            throw;
        }
        catch (Exception exception)
        {
    ...
    private static Task DeleteSulfurLauncherMcTemporaryLoaderAsync(string? instancesRoot, string installFolder,
        string instanceId)
    {
        ...
        string folder, string instanceId)
    {
        ...
        try
        {
            ...
            {
                ImportSulfurLauncherSettings(instancePath);
                step.ReportProgress(1);
                return Task.CompletedTask;
            });
            await RunStepAsync(context, CommonLanguageManager.Instance.minecraft_refreshInstancesStep.CurrentValue(),
        ...
        string archivePath, string? javaPath, string? instancesRoot)
    {
        ...
        {
            if (hasLoader)
                await MoveSulfurLauncherMcInstanceAsync(context, folder, effectiveLoaderId, id, instancesRoot);
            else
                WriteSulfurLauncherMcVanillaInstanceAsync(context, instancesRoot, id, id);
        }

        return minecraft;
    }

    private static void WriteSulfurLauncherMcVanillaInstanceAsync(TaskExecutionContext context, string instancesRoot,
    ...
    private static void ImportSulfurLauncherSettings(string instancePath)
    {
        var portalFolder = Path.Combine(instancePath, "SulfurLauncher");
        if (!Directory.Exists(portalFolder))
            return;

        try
        {
            var exportedConfigPath = Path.Combine(portalFolder, MinecraftInstance.PortableSulfurLauncherConfigFileName);
            if (File.Exists(exportedConfigPath))
            {
                var targetConfigPath = Path.Combine(instancePath, MinecraftInstance.PortableSulfurLauncherConfigFileName);
                ...
                if (!string.IsNullOrEmpty(configDirectory))
                {
                    ...
                    Logger.Info($"[Modpack] Imported SulfurLauncher settings to {targetConfigPath}.");
                }
            }

            var exportedIconPath = Path.Combine(portalFolder, MinecraftInstance.PortableSulfurLauncherIconFileName);
            if (File.Exists(exportedIconPath))
            {
                var targetIconPath = Path.Combine(instancePath, MinecraftInstance.PortableSulfurLauncherIconFileName);
                ...
                if (!string.IsNullOrEmpty(iconDirectory))
                {
                    ...
                    Logger.Info($"[Modpack] Imported SulfurLauncher icon to {targetIconPath}.");
                }
            }
        ...
        {
            Logger.Warning($"[Modpack] Failed to import SulfurLauncher settings from {portalFolder}: {exception}");
        }
    }

    private static Task MoveSulfurLauncherMcInstanceAsync(TaskExecutionContext context, string metadataRoot,
