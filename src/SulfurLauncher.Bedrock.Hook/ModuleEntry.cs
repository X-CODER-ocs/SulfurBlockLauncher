...
using SulfurLauncher.Bedrock.Hook.Mods;
using SulfurLauncher.Bedrock.Hook.Network;
...
/// 与显式导出 HookInit（由 SulfurLauncher.Preload 在 LoadLibraryW 后调用），
/// 两者均只触发一次，确保工作线程被拉起。
/// </summary>
internal static unsafe class ModuleEntry
{
    ...
    private static void WriteBootMarker(string state)
    {
        try
        {
            ...
            string path = Path.Combine(directory, "config", "SulfurLauncher", "logs", "hook-boot.log");
    ...
    private static nint GetSelfModuleBase()
    {
        try
        {
            return NativeMethods.GetModuleHandleW("SulfurLauncher.XUserHook.dll");
        }
        catch
        {
            return 0;
    ...
    private static uint WorkerThread(nint parameter)
    {
        ...
        XUserBridge.Info("SulfurLauncher.XUserHook worker started");
        XUserBridge.Info($"SulfurLauncher.XUserHook base=0x{GetSelfModuleBase():X} pid={Environment.ProcessId}");
