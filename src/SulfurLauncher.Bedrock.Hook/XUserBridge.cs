...
internal static class XUserBridge
{
	...
	private unsafe static int QueryApiHook(Guid* runtimeClassId, Guid* interfaceId, void** outPtr)
	{
		...
		*outPtr = null;
		if (*runtimeClassId == XUserAbi.ClsidXUserImpl)
		{
			...
			Info("QueryApiImpl 已请求 CLSID_XUserImpl；返回 SulfurLauncher 内置 XUser | xbox_gamertag=" + text + " | iid=" + (*interfaceId));
	...
	{
		NativeMethods.OutputDebugStringW("[SulfurLauncher XUser] " + message);
