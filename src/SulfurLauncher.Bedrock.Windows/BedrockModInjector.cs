...
{
    private const string ResourceName = "SulfurLauncher.Inject.dll";
    private const string DllName = "SulfurLauncher.Inject.dll";
    ...
    private static InjectDelegate GetInjector()
    {
        ...
        lock (SyncRoot)
        {
            ...
            var nativePath = Path.Combine(nativeFolder,
                $"SulfurLauncher.Inject-{Convert.ToHexString(SHA256.HashData(bytes))[..16]}.dll");
