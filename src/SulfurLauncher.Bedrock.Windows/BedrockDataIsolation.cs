...
{
    private const string PreloadDllName = "SulfurLauncher.Preload.dll";
    private const string PreviousPreloadDllName = "SulfurLauncher.Preload.Net.dll";
    private const string PreloadResourceName = "SulfurLauncher.Preload.dll";
    private const string FallbackPreloadDllPrefix = "P";
    private const string LegacyPreloadHookName = "XUserHook.dll";
    private const string BootstrapDllName = "SulfurLauncher.Bootstrap.dll";
    ...
    {
        var originalPath = Path.Combine(instancePath, "config", "SulfurLauncher", OriginalExecutableName);
        if (File.Exists(originalPath) && HasSulfurLauncherImportSection(gameExecutable))
            return;
        Directory.CreateDirectory(Path.GetDirectoryName(originalPath)!);
        File.Copy(gameExecutable, originalPath, true);
    }

    private static bool HasSulfurLauncherImportSection(string gameExecutable)
    {
        ...
        Action<string, BedrockLogLevel>? log)
    {
        ...
        peFile.Flush();
        log?.Invoke("Repaired duplicate SulfurLauncher import sections from an interrupted launch preparation.",
    ...
    private static void DeployBootstrapDll(string instancePath)
    {
        ...
        using var stream = assembly.GetManifestResourceStream(BootstrapDllName)
                           ?? throw new InvalidOperationException("SulfurLauncher.Bedrock.Windows does not include SulfurLauncher.Bootstrap.dll.");
    ...
    private static void PrepareLaunchInfo(BedrockInstanceConfig config, Action<string, BedrockLogLevel>? log)
    {
        try
        {
            var outputDirectory = Path.Combine(config.InstancePath, "config", "SulfurLauncher", "launch-info");
            ...
            var launchText = $"©Mojang AB· SulfurLauncher {version}";
            var languageCount = 0;
            ...
            log?.Invoke($"Prepared SulfurLauncher launch information for {languageCount} languages.",
                BedrockLogLevel.Information);
        }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException)
        {
            log?.Invoke($"SulfurLauncher launch information is unavailable: {exception.Message}", BedrockLogLevel.Warning);
        }
    }
    ...
    {
        var runtimeFolder = Path.Combine(config.InstancePath, "preload", "SulfurLauncher");
    ...
    {
        var configFolder = Path.Combine(config.InstancePath, "config", "SulfurLauncher");
