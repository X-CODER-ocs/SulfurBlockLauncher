...
using SulfurLauncher.Bedrock.Core;
using SulfurLauncher.Bedrock.Core.Windows;
...
public class BedrockLaunch : IBedrockLaunch
{
    ...
    private async Task<Process> LaunchWithXboxAccountAsync(BedrockAuthentication account)
    {
        ...
        var launcher = new SulfurLauncherXUserLauncher(_instanceConfig.InstancePath);
        launcher.DeployHook();
        using var authentication = await SulfurLauncherXUserLauncher.AuthenticateAsync(account.AccessToken);
        var executable = Path.Combine(_instanceConfig.InstancePath, "Minecraft.Windows.exe");
        var result = await SulfurLauncherXUserLauncher.LaunchAndInjectAsync(executable, BuildLaunchArguments(),
