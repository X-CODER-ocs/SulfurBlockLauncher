...
internal static unsafe class ModuleEntry
{
    ...
    private static void WriteBootMarker(string state)
    {
        try
        {
            ...
            string path = Path.Combine(directory, "config", "SulfurLauncher", "logs", "boot.log");
    ...
    private static uint WorkerThread(nint parameter)
    {
        try
        {
            ...
            if (console)
            {
                PrintBanner();
                VersionInfo.Print();
                Logger.Success("SulfurLauncher is free software licensed under GPLv3");
                Logger.Success("Submit issues and submit PR: https://github.com/cangcang/sulfurlauncher");
            }
            PreloadLoader.Run();
        }
        catch (Exception ex)
