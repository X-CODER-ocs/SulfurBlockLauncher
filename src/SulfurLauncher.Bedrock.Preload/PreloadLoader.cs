...
{
    private const string XUserHook = "SulfurLauncher.XUserHook.dll";
    ...
    private static IEnumerable<string> EnumerateCandidates(string directory)
    {
        ...
        {
            Logger.Info("Loading SulfurLauncher Xbox account hook first");
            yield return xUserHookPath;
        }
        else
        {
            Logger.Info("SulfurLauncher Xbox account hook is not present; using the default Xbox session");
        }
        ...
        string manifest = Path.Combine(directory, "SulfurLauncher", "mods.txt");
        ...
        foreach (string line in File.ReadLines(manifest))
        {
            ...
            if (name.Length > 0 && Path.GetFileName(name) == name)
                yield return Path.Combine(directory, "SulfurLauncher", name);
        }
    }
    ...
    private static void TriggerXUserHook(nint module)
    {
        ...
        if (hookInit == 0)
        {
            Logger.Warning($"SulfurLauncher.XUserHook.dll 未导出 HookInit；跳过触发");
            return;
        }

        Logger.Info("Calling SulfurLauncher.XUserHook!HookInit");
