...
internal static partial class SulfurLauncherCommandService
{
    ...
    public static bool TryHandleStartupArgs(string[] args)
    {
        switch (SulfurLauncherCommandParser.Parse(args, out var command, out var error))
        {
            ...
            case SulfurLauncherCliParseStatus.Help:
                WriteConsole(SulfurLauncherCommandParser.GetHeadlessUsageText());
                return true;
    ...
    internal static bool TryForwardToRunningInstance(SulfurLauncherCommand command, int attempts = 1)
    {
    ...
    private static bool TryForwardOnce(SulfurLauncherCommand command)
    {
        try
        {
        ...
        while (true)
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(json))
                    continue;
                if (JsonSerializer.Deserialize<SulfurLauncherCommand>(json) is not { } command) continue;
