...
internal static class CliHeadlessLauncher
{
    ...
    public static int Run(SulfurLauncherCommand command)
    {
    ...
    private static async Task<int> LaunchJavaAsync(MinecraftInstance instance, MinecraftEntry entry,
        CliSettings settings, SulfurLauncherCommand command)
    {
        HttpUtil.Configure(false, null, "SulfurLauncher/CLI");
        ...
        var mcProcess = await new Iridium.Launch.Launcher(
            new SulfurLauncherGameArgumentParser(new ArgumentParser(), extraGameArguments))
    ...
    private static MinecraftInstance? ResolveLaunchInstance(SulfurLauncherCommand command, List<MinecraftFolderEntry> folders)
    {
    ...
    private static CliSettings LoadSettings()
    {
        ...
        var settingsPath = SulfurLauncher.Core.Const.ConfigPath.SettingDataPath;
        if (!File.Exists(settingsPath))
            return settings;

        try
        {
            var loaded = JsonSerializer.Deserialize<CliSettings>(File.ReadAllText(settingsPath), SulfurLauncherJson.Options)
                         ?? settings;
    ...
    private static LaunchConfig CreateLaunchConfig(MinecraftInstance instance, Account account, JavaEntry java,
        MinecraftLaunchOptions options, SulfurLauncherCommand command, Dictionary<string, string> placeholders,
    ...
    {
        SulfurLauncherCommandService.WriteConsoleLines(lines);
    }
}
