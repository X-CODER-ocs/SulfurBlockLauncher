...
{
    private const string BootstrapName = "SulfurLauncher.Bootstrap.dll";
    private const string PreloadName = "SulfurLauncher.Preload.dll";
    ...
    public static void Prepare(BedrockInstanceConfig config, Action<string, BedrockLogLevel>? log = null)
    {
        ...
        var portalFolder = Path.Combine(config.InstancePath, "config", "SulfurLauncher");
        ...
        DeployResource("SulfurLauncher.Preload.dll", Path.Combine(config.InstancePath, PreloadName));
        DeployResource("SulfurLauncher.Bootstrap.dll", Path.Combine(config.InstancePath, BootstrapName));
        WriteConfig(config);
    ...
    {
        var output = Path.Combine(config.InstancePath, "config", "SulfurLauncher", "launch-info");
        ...
        var copyright = $"©Mojang AB· SulfurLauncher {version.Replace('\r', ' ').Replace('\n', ' ')}";
        var count = 0;
        ...
        log?.Invoke($"Prepared SulfurLauncher launch information for {count} languages.", BedrockLogLevel.Information);
    }

    private static void WriteConfig(BedrockInstanceConfig config)
    {
        var folder = Path.Combine(config.InstancePath, "config", "SulfurLauncher");
