...
{
    [ObservableProperty]
    public partial Dictionary<string, string> Bindings { get; set; } = ShortcutActionDefaults.CreateDefaultBindings();

    public string? GetGesture(ShortcutAction action)
    {
        return Bindings.TryGetValue(action.ToString(), out var value) && !string.IsNullOrWhiteSpace(value)
            ? value
            : null;
    }
