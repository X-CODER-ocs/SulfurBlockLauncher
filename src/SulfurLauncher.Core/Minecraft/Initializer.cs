...
public static class MinecraftCoreInitializer
{
    ...
    public static void Initialize(MinecraftCoreInitializeOptions options)
    {
        ...
        InitializeHelper.Initialize(settings =>
        {
            ...
            settings.UserAgent = $"SulfurLauncher/{options.AppVersion}";
        ...
        else
        {
            HttpUtil.FlurlClient.Headers.AddOrReplace("User-Agent",
                $"SulfurLauncher/{options.AppVersion}");
            Logger.Debug(string.Format(LogLanguageManager.Instance.minecraft_defaultUserAgentApplied.CurrentValue(), $"SulfurLauncher/{options.AppVersion}"));
        }
