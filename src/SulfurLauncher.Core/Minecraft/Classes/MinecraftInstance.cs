...
using SulfurLauncher.Bedrock.Standard.Manifest;
using SulfurLauncher.Core.Classes;
using SulfurLauncher.Core.Json;
using SulfurLauncher.Core.Minecraft.Graphics;
...
{
    public const string PortableSulfurLauncherConfigFileName = "SulfurLauncher.config.json";

    public const string PortableSulfurLauncherIconFileName = "SulfurLauncher.Icon.png";
    ...
    public bool RequiresIndependentInstance => Layout?.Kind is
        ...
        MinecraftFolderKind.SulfurLauncherMc;
    ...
    public bool CanModifyVersion =>
        ...
        Layout?.Kind == MinecraftFolderKind.SulfurLauncherMc;
    ...
    {
        (true, false) => CommonLanguageManager.Instance.minecraft_bedrockScopeSulfurLauncherInstance.CurrentValue(),
        (false, false) => CommonLanguageManager.Instance.minecraft_bedrockScopeSulfurLauncher.CurrentValue(),
    ...
    private MinecraftInstanceConfig GetInstanceConfig()
    {
        ...
        if (File.Exists(configPath))
            try
            {
                var loadedConfig = Type == MinecraftInstanceType.Java
                    ? JsonSerializer.Deserialize<JavaInstanceConfig>(File.ReadAllText(configPath), SulfurLauncherJson.Options)
                    : JsonSerializer.Deserialize<MinecraftInstanceConfig>(File.ReadAllText(configPath), SulfurLauncherJson.Options);
        ...
        File.WriteAllText(configPath, JsonSerializer.Serialize(config, config.GetType(), SulfurLauncherJson.Options));
        return config;
    }
    ...
    {
        return Path.Combine(InstanceFolderPath, PortableSulfurLauncherConfigFileName);
    }
    ...
    public IReadOnlyList<string> GetDeletionPaths()
    {
        ...
        if (layout.Kind == MinecraftFolderKind.SulfurLauncherMc && MinecraftEntry is { } entry)
        {
    ...
    {
        return Path.Combine(GetSpecialFolder(MinecraftSpecialFolder.InstanceFolder), PortableSulfurLauncherIconFileName);
    }
    ...
    private static Bitmap LoadBitmapFromAssembly(string fileName, int width)
    {
        ...
        var resourcePath = $"SulfurLauncher.Core.Assets.McIcons.{fileName}";

        using var stream = assembly.GetManifestResourceStream(resourcePath);
        if (stream == null)
        {
            var defaultPath = "SulfurLauncher.Core.Assets.McIcons.01_grass_block_side.png";
...
public partial class MinecraftInstanceConfig : ObservableObject
{
    ...
    [ObservableProperty] public partial SulfurLauncherVisibleMode SulfurLauncherVisibleMode { get; set; } = SulfurLauncherVisibleMode.NoOperation;
