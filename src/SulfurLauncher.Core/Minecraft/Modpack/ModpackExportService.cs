...
namespace SulfurLauncher.Core.Minecraft.Modpack;
...
public sealed record ModpackExportOptions
{
    ...
    public bool IncludeSulfurLauncherSettings { get; init; } = true;
    public bool IncludeSulfurLauncherIcon { get; init; } = true;
}
...
public sealed class ModpackExportService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheFolder = Path.Combine(Path.GetTempPath(), "SulfurLauncher", "ModpackExport",
        ...
        try
        {
            ...
            if (options.IncludeSulfurLauncherSettings)
                CopySulfurLauncherSettings(instance, overridesFolder);

            if (options.IncludeSulfurLauncherIcon)
                CopySulfurLauncherIcon(instance, overridesFolder);
    ...
    private static void CopySulfurLauncherSettings(MinecraftInstance instance, string overridesFolder)
    {
        ...
        try
        {
            var targetDirectory = Path.Combine(overridesFolder, "SulfurLauncher");
            Directory.CreateDirectory(targetDirectory);
            File.Copy(configPath, Path.Combine(targetDirectory, MinecraftInstance.PortableSulfurLauncherConfigFileName), true);
        }
        catch (Exception exception)
        {
            Logger.Warning(string.Format(LogLanguageManager.Instance.modpack_copySulfurLauncherSettingsFailed.CurrentValue(), exception));
        }
    }

    private static void CopySulfurLauncherIcon(MinecraftInstance instance, string overridesFolder)
    {
        ...
        try
        {
            var targetDirectory = Path.Combine(overridesFolder, "SulfurLauncher");
            Directory.CreateDirectory(targetDirectory);
            File.Copy(iconPath, Path.Combine(targetDirectory, MinecraftInstance.PortableSulfurLauncherIconFileName), true);
        }
        catch (Exception exception)
        {
            Logger.Warning(string.Format(LogLanguageManager.Instance.modpack_copySulfurLauncherIconFailed.CurrentValue(), exception));
        }
    }
