...
namespace SulfurLauncher.Core.Minecraft.Instance.Bedrock;
...
public static class BedrockDataPathResolver
{
    public static string GetDataRoot(BedrockInstanceConfig config)
    {
        ...
        if (config.EnableIndependentInstance)
            return config.EnableLauncherSharedData
                ...
                : GetSulfurLauncherIsolationRoot(config.InstancePath);

        return config.EnableLauncherSharedData ? GetWindowsDataRoot(config) : GetSulfurLauncherDataRoot();
    }

    private static string GetSulfurLauncherIsolationRoot(string instancePath)
    {
        return Path.Combine(instancePath, "config", "SulfurLauncher", "isolation");
    }
        ...
        if (string.IsNullOrWhiteSpace(prefix))
        {
            ...
            if (string.IsNullOrWhiteSpace(dataHome))
                dataHome = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".local",
                    "share");
            prefix = Path.Combine(dataHome, "SulfurLauncher", "Bedrock", "proton-prefix");
        }
    ...
    public static void EnsureSulfurLauncherDataDirectories()
    {
        var mojangRoot = Path.Combine(GetSulfurLauncherDataRoot(), "Users", "Shared", "games", "com.mojang");
