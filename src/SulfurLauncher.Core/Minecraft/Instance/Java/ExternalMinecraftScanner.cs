...
internal static class ExternalMinecraftScanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return layout.Kind switch
        {
            ...
            MinecraftFolderKind.SulfurLauncherMc => await ScanSulfurLauncherMcAsync(folder, layout, cancellationToken),
            _ => []
        };
    }

    private static async Task<IReadOnlyList<MinecraftInstance>> ScanSulfurLauncherMcAsync(MinecraftFolderEntry folder,
        MinecraftFolderLayout folderLayout, CancellationToken cancellationToken)
    {
        var result = new List<MinecraftInstance>();
        result.AddRange(await ScanSulfurLauncherMcJavaAsync(folder, folderLayout, cancellationToken));
        result.AddRange(ScanSulfurLauncherMcBedrock(folder, folderLayout));
        return result;
    }

    private static async Task<IReadOnlyList<MinecraftInstance>> ScanSulfurLauncherMcJavaAsync(MinecraftFolderEntry folder,
        MinecraftFolderLayout folderLayout, CancellationToken cancellationToken)
    {
        ...
        foreach (var context in await new MinecraftProvider(new DirectoryInfo(root),
                     [new SulfurLauncherMcProvider()]).GetMinecraftsAsync(cancellationToken))
        {
            try
            {
                var entry = context.Entry;
                var iconPath = ResolveIcon(entry.InstancePath, "icon.png") ?? ResolveIcon(entry.InstancePath, "Icon.png")
                    ?? ResolveIcon(entry.InstancePath, "SulfurLauncher.Icon.png");
                result.Add(CreateInstance(context, folder, MinecraftFolderKind.SulfurLauncherMc, root, metadataRoot,
                    iconPath, entry.Name));
            }
    ...
    private static IReadOnlyList<MinecraftInstance> ScanSulfurLauncherMcBedrock(MinecraftFolderEntry folder,
