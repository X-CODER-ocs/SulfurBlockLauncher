...
using Data = SulfurLauncher.Core.Const.Data;
...
public static class MinecraftInstallationTasks
{
    ...
    {
        if (folder.DetectedLayout.Kind == MinecraftFolderKind.SulfurLauncherMc)
        {
            await RunSulfurLauncherMcInstallationAsync(context, vanilla, folder, versionId, selectedEntries, javaPath);
            return;
        }
    ...
    private static async Task RunSulfurLauncherMcInstallationAsync(TaskExecutionContext context, VersionManifestEntry vanilla,
        MinecraftFolderEntry folder, string versionId,
        IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath)
    {
        ...
        try
        {
                ...
                CommonLanguageManager.Instance.minecraft_generatingInstanceConfig.CurrentValue(), step =>
            {
                ...
                {
                    WriteSulfurLauncherMcMinimalInstanceJson(instanceDirectory, versionId, vanillaId);
                }
    ...
    public static void WriteSulfurLauncherMcMinimalInstanceJson(string instanceDirectory, string instanceId, string vanillaId)
    {
