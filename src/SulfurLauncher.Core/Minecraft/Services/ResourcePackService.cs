...
public sealed class ResourcePackService
{
    ...
    {
        return Task.Run(() => Scan(instance, folder, cancellationToken), cancellationToken);
    }
    ...
    private static ResourcePackInfo ReadJavaPack(string path, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var icon = archive.Entries.FirstOrDefault(entry =>
                           entry.FullName.Equals("pack.png", StringComparison.OrdinalIgnoreCase)) ??
                       archive.Entries.FirstOrDefault(entry =>
                           entry.FullName.Equals("logo.png", StringComparison.OrdinalIgnoreCase)) ??
                       archive.Entries.FirstOrDefault(entry =>
    ...
    private static ResourcePackInfo ReadBedrockPack(string path, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (root.TryGetProperty("metadata", out var metadata) && metadata.ValueKind == JsonValueKind.Object &&
                metadata.TryGetProperty("authors", out var authorValues) &&
                authorValues.ValueKind == JsonValueKind.Array)
            ...
            if (root.TryGetProperty("subpacks", out var subpackValues) &&
                subpackValues.ValueKind == JsonValueKind.Array)
            ...
            if (root.TryGetProperty("capabilities", out var capabilityValues) &&
            ...
            if (root.TryGetProperty("dependencies", out var dependencyValues) &&
                dependencyValues.ValueKind == JsonValueKind.Array)
                dependencies.AddRange(dependencyValues.EnumerateArray()
        ...
        return new ResourcePackInfo(path, fileName, displayName, description, version,
            ...
            directory.LastWriteTime, ReadIcon(Path.Combine(path, "pack_icon.png")), true, minEngineVersion, uuid,
            authors, subpacks, capabilities, modules, dependencies);
    }
    ...
    private static ResourcePackInfo? ReadBedrockSkinPack(string path, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var skinPackModules = moduleValues.EnumerateArray().Where(module =>
    ...
    private static string? GetVersion(JsonElement element, string property)
    {
        ...
        var version = string.Join('.',
            value.EnumerateArray().Where(part => part.ValueKind == JsonValueKind.Number)
    ...
    private static string? GetFormats(JsonElement element)
    {
