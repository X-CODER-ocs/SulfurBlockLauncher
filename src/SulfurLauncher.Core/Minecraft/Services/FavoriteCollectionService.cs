...
public sealed class FavoriteCollectionService
{
    ...
    public void Save()
    {
        ...
        File.WriteAllText(temporaryPath, JsonSerializer.Serialize(Document, SulfurLauncherJson.Options));
    ...
    {
        var imported = JsonSerializer.Deserialize<FavoriteCollectionDocument>(File.ReadAllText(path), SulfurLauncherJson.Options);
    ...
    public void Export(FavoriteCollection collection, string path)
    {
        ...
        File.WriteAllText(path, JsonSerializer.Serialize(document, SulfurLauncherJson.Options));
    }
    ...
    private FavoriteCollectionDocument Load()
    {
        try
        {
            return File.Exists(_path)
                ? JsonSerializer.Deserialize<FavoriteCollectionDocument>(File.ReadAllText(_path), SulfurLauncherJson.Options) ??
