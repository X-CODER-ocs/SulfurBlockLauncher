...
{
    private const string BackupFolderName = ".SulfurLauncherModifyBackup";
        ...
        VersionManifestEntry vanilla, IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath)
    {
        ...
        if (!instance.CanModifyVersion)
            throw new InvalidOperationException(CommonLanguageManager.Instance.versionModify_onlySulfurLauncher.CurrentValue());
        ...
        var isSulfurLauncherMc = MinecraftFolderLayout.TryFindSulfurLauncherMcRoot(folderPath, out var portalMcRoot);
        var metadataRoot = isSulfurLauncherMc ? Path.Combine(portalMcRoot, "meta") : folderPath;
        var instanceDirectory = isSulfurLauncherMc
            ? instance.Layout?.InstanceRoot ?? Path.Combine(portalMcRoot, "instances", instanceId)
            : Path.Combine(folderPath, "versions", instanceId);
        var hasLoaders = selectedEntries.Count > 0;
        var usesTempBase = !isSulfurLauncherMc && hasLoaders &&
        ...
        try
        {
            ...
            {
                if (isSulfurLauncherMc)
                    await InstallLoadersSulfurLauncherMcAsync(context, metadataRoot, instanceDirectory, instanceId, vanilla,
            ...
            {
                if (isSulfurLauncherMc)
                    await InstallPureVanillaSulfurLauncherMcAsync(context, metadataRoot, instanceDirectory, instanceId, vanilla,
                        sourceRoots);
                else
    ...
    private static async Task InstallPureVanillaSulfurLauncherMcAsync(TaskExecutionContext context, string metadataRoot,
        string instanceDirectory, string instanceId, VersionManifestEntry vanilla, IEnumerable<string> sourceRoots)
    {
        await Task.Run(() =>
        {
        ...
        {
            MinecraftInstallationTasks.WriteSulfurLauncherMcMinimalInstanceJson(instanceDirectory, instanceId, vanilla.Id);
            step.ReportProgress(1);
            return Task.CompletedTask;
        });
    }

    private static async Task InstallLoadersSulfurLauncherMcAsync(TaskExecutionContext context, string metadataRoot,
        string instanceDirectory, string instanceId, VersionManifestEntry vanilla,
        IReadOnlyDictionary<LoaderKind, IInstallEntry> selectedEntries, string? javaPath,
        IEnumerable<string> sourceRoots)
    {
