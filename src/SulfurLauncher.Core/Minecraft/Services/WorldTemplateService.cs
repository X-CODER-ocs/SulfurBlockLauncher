...
public sealed class WorldTemplateService
{
    ...
    private static IReadOnlyList<WorldTemplateInfo> Scan(MinecraftInstance instance,
        CancellationToken cancellationToken)
    {
        try
        {
            return Directory
                .EnumerateDirectories(instance.GetSpecialFolder(MinecraftSpecialFolder.WorldTemplatesFolder))
    ...
    private static WorldTemplateInfo? Read(string path, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var templateModules = moduleValues.EnumerateArray().Where(module =>
                    module.ValueKind == JsonValueKind.Object &&
                    string.Equals(GetString(module, "type"), "world_template", StringComparison.OrdinalIgnoreCase))
                .ToArray();
        ...
        return new WorldTemplateInfo(path, fileName, displayName, description, version, baseGameVersion, uuid, modules,
            ReadIcon(Path.Combine(path, "world_icon.jpeg")),
    ...
    private static IReadOnlyList<WorldTemplatePackReference> ReadPackReferences(string path)
    {
        ...
        try
        {
            ...
            return document.RootElement.EnumerateArray().Where(value => value.ValueKind == JsonValueKind.Object)
                .Select(value => new WorldTemplatePackReference(GetString(value, "pack_id"),
                    GetString(value, "subpack"), GetVersion(value, "version")))
...
public sealed record WorldTemplateInfo(
    string FilePath,
    string FileName,
    string DisplayName,
    string? Description,
    ...
    string? Uuid,
    IReadOnlyList<string> ModuleUuids,
    byte[]? IconData,
    IReadOnlyList<WorldTemplatePackReference> BehaviorPacks,
    IReadOnlyList<WorldTemplatePackReference> ResourcePacks);
