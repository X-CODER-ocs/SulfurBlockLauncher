...
public static class ProtocolRegistration
{
    private const string WindowsScriptTemplate =
        ...
        @echo off
        chcp 65001 >nul
        reg add "HKEY_CLASSES_ROOT\portal" /f /ve /d "URL:SulfurLauncher Protocol" || goto :fail
    ...
    private const string LinuxDesktopTemplate =
        ...
        Type=Application
        Name=SulfurLauncher
        Comment=SulfurLauncher URL protocol handler
    ...
    private static async Task RegisterWindowsAsync(string executablePath)
    {
        ...
        var scriptPath = Path.Combine(ConfigPath.TempFolderPath, "RegisterSulfurLauncherProtocol.bat");
