...
namespace SulfurLauncher.Core.Module.Update;
...
public static class UpdateApp
{
    ...
    internal static UpdateAsset SelectAsset(UpdateRelease release, string packageType)
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            {
                "installer" => "SulfurLauncher.win.x64.installer.zip",
                "portable" => "SulfurLauncher.win.x64.portable.zip",
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            expectedName = packageType switch
            {
                "appimage" => "SulfurLauncher.linux.x64.AppImage",
                "deb" => "SulfurLauncher.linux.x64.deb",
                "rpm" => "SulfurLauncher.linux.x64.rpm",
        ...
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            ...
            expectedName = $"SulfurLauncher.osx.mac.{arch}.app.zip";
        }
        else
        {
    ...
    private static ProcessStartInfo PrepareWindowsPortable(string zipPath, string updateDirectory, string target)
    {
        ...
        File.WriteAllText(script, $$"""
                                    ...
                                    try {
                                      ...
                                      if (Get-Process -Id $pidToWait -ErrorAction SilentlyContinue) { throw 'SulfurLauncher did not exit in time.' }
                                      ...
                                      if ($process.HasExited) { throw 'The updated SulfurLauncher exited immediately.' }
    ...
    private static PreparedUpdate PrepareAppImage(string packagePath, string updateDirectory)
    {
        ...
        File.WriteAllText(workerScript, $$"""
                                          ...
                                          log={{Sh(log)}}
                                          exec >>"$log" 2>&1
                                          echo "SulfurLauncher AppImage update started: $(date -Is)"
                                          ...
                                          if [ -n "${PKEXEC_UID:-}" ]; then
                                            ...
                                            if ! command -v runuser >/dev/null 2>&1; then
                                              echo "The system does not provide runuser; SulfurLauncher was replaced but not restarted."
                                              exit 1
                                            fi
                                            runuser -u "$user" -- env \
                                              HOME="$home" \
                                          ...
                                          sleep 5
                                          rm -rf "$backup"
                                          echo "SulfurLauncher AppImage update completed: $(date -Is)"
                                          """);
        File.WriteAllText(launcherScript, $$"""
                                              ...
                                              log={{Sh(log)}}
                                              echo "SulfurLauncher update authentication accepted: $(date -Is)" >>"$log"
        ...
        string packagePath, string updateDirectory, string packageType, string processPath)
    {
        ...
        File.WriteAllText(workerScript, $$"""
                                          ...
                                          log={{Sh(log)}}
                                          exec >>"$log" 2>&1
                                          echo "SulfurLauncher package update started: $(date -Is)"
                                          pid='{{Environment.ProcessId}}'
                                          ...
                                          echo "Starting the updated SulfurLauncher process as $user."
                                          if ! command -v runuser >/dev/null 2>&1; then
                                            echo "The system does not provide runuser; SulfurLauncher was installed but not restarted."
                                            exit 1
                                          fi
                                          runuser -u "$user" -- env \
                                            HOME="$home" \
                                            ...
                                            XDG_RUNTIME_DIR="$xdg_runtime_dir" \
                                            nohup "$target" >/dev/null 2>&1 &
                                          echo "SulfurLauncher package update completed: $(date -Is)"
                                          """);
        File.WriteAllText(launcherScript, $$"""
                                            #!/bin/sh
                                            set -eu
    ...
    private static ProcessStartInfo PrepareMacApp(string packagePath, string updateDirectory, string processPath)
    {
        ...
        var replacement = Path.Combine(extracted, "SulfurLauncher.app");
        if (!File.Exists(Path.Combine(replacement, "Contents", "MacOS", "SulfurLauncher.Desktop")))
