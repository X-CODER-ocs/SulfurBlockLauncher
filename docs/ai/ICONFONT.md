...
Copy-Item "<tmp>\font_*\iconfont.json" "E:\SulfurLauncher\docs\ai\iconfont.json" -Force
Copy-Item "<tmp>\font_*\iconfont.ttf" "E:\SulfurLauncher\src\SulfurLauncher\Assets\Fonts\iconfont.ttf" -Force
```
编辑 `src\SulfurLauncher\Module\IconResources.cs` 的 `Glyphs` 字典：
...
- 窗口按钮：`src\SulfurLauncher\Styles\Style.axaml` 的 `tio|TioTitleBar` Setter `Value="&#xXXXX;"`

重建验证：
```powershell
dotnet build "E:\SulfurLauncher\src\SulfurLauncher.Desktop\SulfurLauncher.Desktop.csproj" -c Debug
```

## 渲染图标
XAML：
```xml
xmlns:icons="using:SulfurLauncher.Module"
...
## TTF 更换
- 嵌入资源（`avares://SulfurLauncher/Assets/Fonts/iconfont.ttf`）：替换文件 + 重建即可
